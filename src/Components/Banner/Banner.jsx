...
const Banner = ({ MainTitle, Subtitle, btnFrom, textFrom, textTo, radialFrom, radialTo, btnTo, btnColr, borderColor, contactFrom, contactTo }) => {
    ...
    return (
        <div className='w-full h-auto  flex flex-col bg-transparent  relative overflow-hidden '>
            <img src={`${store.currentPage == 'MainPage' ? FTAMainBanner
                : store.currentPage == 'ArtOfInvestment' ? ArtOfInvestmentBanner
                    : store.currentPage == 'FutureTrading' ? ExpertFutureTradingBanner
                        : store.currentPage == 'OptionTrading' ? ExpertOptionTradingBanner
                            : store.currentPage == 'SMBeginners' ? StockMarketBeginnerBannerImage
                                : store.currentPage == 'OnlineCourse' ? OnlineCourseBanner
                                    : store.currentPage == 'IntraDay' ? IntradayBanner
                                        : store.currentPage == 'AboutUs' ? AboutUsBanner
                                            : 'none'}`} alt="" className=' absolute z-10 w-fit lg:w-full h-full top-0 bannerImage  object-cover ' />
                ...
                <div className={`w-full h-auto  py-14 relative z-999 flex flex-col ${store.currentPage == 'MainPage' ? '  sm:gap-80 lg:gap-24' : 'gap-12'}  `}>
                    <div className='w-full h-auto flex flex-col relative gap-8 items-center pb-20 lg:pb-52  '>
                        ...
                        <div className='w-full h-auto flex items-center justify-center'>
                            <div className='w-fit flex flex-col gap-4 px-4 lg:px-0 items-center justify-center text-[#E7FFF8]'>
                                <h1 className={`leading-7 flex justify-center lg:leading-10 ${store.currentPage == 'MainPage' ? ' w-full md:w-[80%] ' : 'w-full'}`}><span className='Alatsi font-medium text-[1.7rem] md:text-4xl lg:text-5xl text-center text-transparent bg-clip-text ' style={{ backgroundImage: `radial-gradient(circle at 50% 90%, ${textFrom}, ${textTo})` }}>{MainTitle ? MainTitle : <span>Become a Pro Trader and Investor by joining our Stock Market Mentorship Program.</span>}</span></h1>
                                <p className={`Chivo font-light text-[.75rem] lg:text-[1rem] text-center ${store.currentPage == 'MainPage' ? ' w-full' : ' md:w-[90%] lg:w-[75%]'}  `}>{Subtitle ? Subtitle : <span>The place where you get 'COLLECTIVE WISDOM' in Financial Literacy to achieve your financial goals.</span>}</p>
                            </div>
                        </div>
                        {/* details */}
                        ...
                        {store.currentPage == 'MainPage' ? <div className=' w-fit h-fit md:absolute md:mt-[30%] md:ml-[65%] xl:mt-[15%] xl:ml-[65%] flex flex-col max-md:gap-2 max-md:items-center  z-1000'>
                    ...
                    <div className='w-full h-auto flex flex-col gap-12 '>
                        ...
                        <div className='w-full h-auto flex flex-col gap-5 md:gap-8 items-center justify-center'>
                            <div className='w-fit h-auto px-4 py-1 border border-[#0FCD65] rounded-full bg-black/30 ' style={{ borderColor: borderColor }}>
                            ...
                            <button className={`Roboto w-fit h-auto px-8 py-1 md:px-18 md:py-3 border-2 rounded-full text-[1rem] md:text-xl lg:text-2xl cursor-pointer font-bold bg-linear-to-r from-[#FBF5FF] to-[#D5FFE2] text-[#0BB759] `} style={{ background: `linear-gradient(to right , ${btnFrom},${btnTo})`, color: btnColr }}
                                onClick={() => store.handleShow()}>Enroll Now</button>
                        </div>
                        {/* data */}
